...
let () =
  ...
  | _ :: ops :: archive :: files ->
      ...
      let existing = if Sys.file_exists archive then Occ.Archiver.Ar.read (read_file archive) else [] in
      let basename f = Filename.basename f in
      let result =
        if has 'r' then
          (* replace members of the same name in place, append the rest *)
          let added = List.map (fun f -> { Occ.Archiver.Ar.name = basename f; body = read_file f }) files in
          let replaced = List.map (fun (m : Occ.Archiver.Ar.member) ->
              match List.find_opt (fun (a : Occ.Archiver.Ar.member) -> a.name = m.name) added with Some a -> a | None -> m) existing in
          let fresh = List.filter (fun (a : Occ.Archiver.Ar.member) -> not (List.exists (fun (m : Occ.Archiver.Ar.member) -> m.name = a.name) existing)) added in
          Some (replaced @ fresh)
        else if has 'q' then
          Some (existing @ List.map (fun f -> { Occ.Archiver.Ar.name = basename f; body = read_file f }) files)
        else if has 'd' then
          Some (List.filter (fun (m : Occ.Archiver.Ar.member) -> not (List.mem m.name (List.map basename files))) existing)
        else if has 't' then begin
          List.iter (fun (m : Occ.Archiver.Ar.member) ->
      ...
      (match result with
       | Some members -> write_file archive (Occ.Archiver.Ar.write members)
       | None -> ())
